...
test("client can complete the A3Pay push payment demo", async ({ page }) => {
  ...
  await page.getByLabel("Номер телефона").fill("+7 900 123-45-67");
  await page.getByRole("button", { name: /Получить push/ }).click();
...
test("merchant can see the client-not-found state", async ({ page }) => {
  ...
  await page.getByRole("button", { name: /Проверить и отправить push/ }).click();
